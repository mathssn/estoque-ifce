...
def recalculate_entries_balance(data, entrada: Entrada, session_db: Session, update=False, delete=False):
    ...
    if qntd_final < 0:
        if delete:
            flash('Não é possivel deletar essa entrada, pois o saldo ficará negativo!', 'danger')
        elif update:
            flash('Não é possivel atualizar essa entrada para esse valor, pois o saldo ficará negativo!', 'danger')
...
def list_movementation_by_product(produto_id, limit, offset):
    query = """
        ...
        ORDER BY data_movimentacao DESC, refeicao_id DESC
